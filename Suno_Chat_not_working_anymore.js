  ...
  setTimeout(() => {
    ...
    function getCloneContentEl(cloneEl) {
      ...
      return (
        ...
        cloneEl.querySelector("span.text-white\\/60") || // (escaped) just in case
        (() => {
          for (const span of cloneEl.querySelectorAll("span")) {
...
#ncz-chat-mirror-host a.ncz-sunoLink{
  text-decoration: underline;
  cursor: pointer;
}
#ncz-chat-mirror-host a.ncz-sunoLink:hover{
...
#ncz-chat-mirror-host span.font-medium{
  ...
  display: inline-block;
    ...
    let encryptSend = !!st0.encryptSend; // NEW: default false (plaintext)
    let hidden = false;
    ...
    // NEW: Encrypt toggle (OFF by default)
    ...
    function updateRoomTag() {
      if (room === 0) {
        roomTag.textContent = "Send: Base chat";
      } else {
        roomTag.textContent = encryptSend ? `Send: ROOM${room}: (encrypted)` : `Send: ROOM${room}: (plain)`;
      }
    }
    updateRoomTag();
    ...
    function linkifySunoInElement(el) {
      ...
      while ((m = SUNO_RE.exec(text)) !== null) {
        ...
        a.href = toHref(raw);
        a.textContent = raw; // keep word-joiner in displayed text
        a.target = "_blank";
    ...
    // ---------------------------------------------------------------------
    // ROOM ENCRYPTION / DECRYPTION (with plaintext @mentions + plaintext mode)
    //
    // - When sending to ROOMx (plain mode):   ROOMx: <plaintext>
    // - When sending to ROOMx (encrypt on):
    //    - If no @mentions:   ROOMx: NCZ1.<iv>.<ciphertext>
    //    - If has @mentions:  ROOMx: ⟦NCZ1...⟧@name⟦NCZ1...⟧
    // - Decoding auto-detects: if payload looks encrypted -> decrypt, else show as-is.
    //
    // Set secret once in DevTools console:
    //   nczSetRoomSecret("long random shared secret")
    // ---------------------------------------------------------------------
    const CRYPTO_LS_KEY = "ncz_chatMirror_roomCrypto_v1";
    const CRYPTO_MARKER = "NCZ1"; // version marker for encrypted payloads

    // Mixed wrappers around encrypted chunks (so @mentions can stay plaintext)
    const ENC_OPEN = "⟦";
    const ENC_CLOSE = "⟧";
    ...
    // quick helper you can call in DevTools console
    window.nczSetRoomSecret = (secret) => {
      saveCryptoState({ masterSecret: String(secret || "") });
      console.log("[NCZ] Room secret saved. Reload Suno.");
    };
    ...
    async function getRoomKey(roomNumber) {
      ...
      const p = (async () => {
        ...
        // derive a per-room 32-byte key from SHA-256(master + "|ROOM|" + rn)
    ...
    async function encryptRoomText(roomNumber, plainText) {
      ...
      // If WebCrypto isn't there, fallback to "opaque" base64 (not real crypto)
    ...
    async function decryptRoomText(roomNumber, payload) {
      ...
      const ivB64 = parts[1] || "";
      const ctB64 = parts.slice(2).join("."); // tolerate extra dots
      try {
    ...
    // decrypt cache for single payloads so renderNow stays fast (no await)
    const decryptCache = new Map(); // key: `${room}|${payload}` -> string|null|"PENDING"
    function getDecryptedOrKick(roomNumber, payload) {
      const rn = Number(roomNumber) || 0;
    ...
    // Encrypt message but keep @mentions plaintext.
    // - If there are no mentions, returns the normal single payload (short + backward compatible).
    // - If there ARE mentions, returns mixed string: ⟦payload⟧@bob⟦payload⟧...
    async function encryptRoomTextKeepMentions(roomNumber, plainText) {
      ...
      if (!hasMention) {
        return await encryptRoomText(roomNumber, safe);
      }
      ...
      for (const p of parts) {
        if (!p.value) continue;
        if (p.type === "mention") {
          out.push(p.value); // plaintext mention
        } else {
          const payload = await encryptRoomText(roomNumber, p.value);
          out.push(`${ENC_OPEN}${payload}${ENC_CLOSE}`);
        }
      }
      return out.join("");
    }

    // Decrypt a mixed message containing 0..N wrapped encrypted chunks.
    // Leaves plaintext (including @mentions) untouched.
    ...
    // Cache for mixed decrypts (renderNow can't await)
    const mixedDecryptCache = new Map(); // key `${room}|${text}` -> string|null|"PENDING"
    function getDecryptedMixedOrKick(roomNumber, mixedText) {
      const rn = Number(roomNumber) || 0;
    ...
    async function sendFromMirror() {
      ...
      if (activeRoom > 0) {
        if (encryptSend) {
          // Encrypt ROOM message body but keep @mentions plaintext (and still obfuscate .com)
          const encrypted = await encryptRoomTextKeepMentions(activeRoom, text);
          msgToSend = `${prefix} ${encrypted}`;
        } else {
          // Plaintext ROOM mode (still obfuscate .com)
          const safeText = obfuscateDotCom(text);
          msgToSend = `${prefix} ${safeText}`;
        }
      } else {
        // Base chat: .com obfuscation only
        const safeText = obfuscateDotCom(text);
        msgToSend = safeText;
      }
    ...
    mirrorSendBtn.addEventListener("click", () => {
      sendFromMirror().catch((e) => console.error("[NCZ] sendFromMirror failed:", e));
    });
    mirrorInput.addEventListener("keydown", (e) => {
      if (e.key === "Enter" && !e.shiftKey) {
        e.preventDefault();
        sendFromMirror().catch((err) => console.error("[NCZ] sendFromMirror failed:", err));
      }
    });
    ...
    function renderNow() {
      ...
      for (const msg of msgNodes) {
        ...
        // If in a room, strip ROOMX: from the mirrored display, then decrypt if needed
        if (activeRoom > 0) {
          ...
          // mixed first (⟦NCZ...⟧ with plaintext @mentions)
          if (looksMixedEncrypted(stripped)) {
            const dec = getDecryptedMixedOrKick(activeRoom, stripped);
            displayText = dec.ready
              ? ((dec.text == null) ? `🔒 (can't decrypt) ${stripped}` : dec.text)
              : dec.text;
          }
          // single payload (NCZ1... or NCZ0...)
          else if (looksEncryptedPayload(candidate) || candidate.startsWith("NCZ0.")) {
            const dec = getDecryptedOrKick(activeRoom, candidate);
            displayText = dec.ready
        ...
        // Make ONLY suno.com links clickable in the mirror (new tab), respecting word-joiner
    ...
    function onRouteMaybeChanged(force = false) {
      ...
      if (!routeActive && now) {
        ...
        // entering /live-radio always re-opens the mirror even if user closed it
        hidden = false;
