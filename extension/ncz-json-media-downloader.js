(() => {
  ...
  function downloadViaBridge(url, filename) {
    return new Promise((resolve, reject) => {
      ...
      function onMessage(event) {
        ...
        if (msg.type !== "NCZ_DOWNLOAD_URL_RESPONSE") return;
        ...
        if (msg.ok) {
          ...
          reject(new Error(msg.error || "Bridge download failed."));
        }
      }
      ...
      setTimeout(() => {
        window.removeEventListener("message", onMessage);
        reject(new Error("Bridge download timed out."));
      }, 15000);
    });
  }
  ...
  async function tryDownload(url, filename, log, current, total, opts = {}) {
    const {
      preferBridge = false,
      noDirectLinkFallback = false
    } = opts;

    if (preferBridge) {
      try {
        await downloadViaBridge(url, filename);
        log(`[${current}/${total}] Downloaded via extension bridge: ${filename}`);
        return true;
      } catch (err) {
        log(`[${current}/${total}] Bridge download failed for ${filename}. (${err.message})`);
      }
    }
    ...
    try {
      ...
      log(`[${current}/${total}] Fetch failed for ${filename}${noDirectLinkFallback ? "." : ", trying direct-link fallback..."} (${err.message})`);
    }

    if (!preferBridge) {
      try {
        await downloadViaBridge(url, filename);
        log(`[${current}/${total}] Downloaded via extension bridge fallback: ${filename}`);
        return true;
      } catch (err) {
        log(`[${current}/${total}] Bridge fallback failed for ${filename}. (${err.message})`);
      }
    }

    if (noDirectLinkFallback) {
      log(`[${current}/${total}] FAILED: ${filename} :: all non-tab download methods failed.`);
      return false;
    }
    ...
    try {
      ...
      log(`[${current}/${total}] FAILED: ${filename} :: ${err.message}`);
      return false;
    }
  }
  ...
  const DOWNLOAD_MODES = {
    ...
    genvideos: {
      ...
      required: ["video_url"],
      files: [
        { key: "video_url", fallbackExt: ".mp4", forceNameSuffix: ".mp3.mp4" }
      ]
    },
    meta: {
      label: "meta",
    ...
    images: {
      label: "images",
      required: ["image_large_url"],
      files: [
        { key: "image_large_url", fallbackExt: ".jpg" }
      ]
    }
  };
  ...
  function openPanel() {
    ...
    panel.innerHTML = `
      ...
      <div style="margin-top:10px;font-size:11px;opacity:.85;">
        ...
        <div><b>Download Generated Videos</b> uses <code>video_url</code> and saves as <code>.mp3.mp4</code>.</div>
        <div><b>Download Meta</b> saves each raw entry as a <code>.json</code> file.</div>
        <div><b>Download Images</b> uses <code>image_large_url</code> and routes through the extension bridge.</div>
      </div>
    ...
    function setButtonsDisabled(disabled) {
      [$fetch, $both, $mp3, $covers, $genvideos, $meta, $images, $clear].forEach(btn => {
    ...
    async function runDownloadMode(modeKey) {
      ...
      try {
        ...
        for (let i = 0; i < valid.length; i++) {
          ...
          if (modeKey === "meta") {
            try {
              const jsonText = JSON.stringify(item.raw, null, 2);
              ...
              log(`[${current}/${total}] FAILED meta: ${base}.json :: ${err.message}`);
            }
            await sleep(250);
            continue;
          }
          ...
          for (const fileDef of mode.files) {
            ...
            const filename = fileDef.forceNameSuffix
              ? `${base}${fileDef.forceNameSuffix}`
              : `${base}${getExtFromUrl(url, fileDef.fallbackExt)}`;

            const opts =
              modeKey === "images"
                ? { preferBridge: true, noDirectLinkFallback: true }
                : {};

            const ok = await tryDownload(url, filename, log, current, total, opts);
            if (!ok) itemOk = false;
            await sleep(250);
          }
