(() => {
  ...
  const START_DELAY_MS = 7500; // shorter; Suno UI is SPA and chat mounts late anyway
  const LAUNCHER_ID = "ncz-chat-mirror-launcher";


  const JSON_DL_EVENT = "ncz-json-media-downloader-open";
  const JSON_DL_SCRIPT_ID = "ncz-json-media-downloader-script";

  function ensureJsonDownloaderLoaded(onReady) {
    const fireReady = () => {
      try {
        onReady && onReady();
      } catch (e) {
        console.error(LOG, "JSON downloader ready callback failed:", e);
      }
    };

    const existing = document.getElementById(JSON_DL_SCRIPT_ID);
    if (existing) {
      if (existing.dataset.loaded === "1") {
        fireReady();
      } else if (onReady) {
        existing.addEventListener("load", fireReady, { once: true });
      }
      return;
    }

    const src = chrome?.runtime?.getURL?.("ncz-json-media-downloader.js");
    if (!src) {
      console.error(LOG, "Could not resolve ncz-json-media-downloader.js");
      return;
    }

    const s = document.createElement("script");
    s.id = JSON_DL_SCRIPT_ID;
    s.src = src;
    s.async = false;
    s.dataset.loaded = "0";
    ...
    s.addEventListener("load", () => {
      s.dataset.loaded = "1";
      console.log(LOG, "JSON downloader script loaded.");
      fireReady();
    }, { once: true });

    s.addEventListener("error", (e) => {
      console.error(LOG, "Failed to load ncz-json-media-downloader.js", e);
    }, { once: true });

    (document.head || document.documentElement).appendChild(s);
  }

  function openJsonDownloaderPanel() {
    ensureJsonDownloaderLoaded(() => {
      window.dispatchEvent(new CustomEvent(JSON_DL_EVENT));
    });
  }
  ...
  const LIVE_RADIO_RE = /^\/(?:labs\/)?live-radio(?:\/.*)?$/i;
...
function isLiveRadioContext() {
  // STRICT: only show on /live-radio (and /labs/live-radio)
  return isLiveRadioUrl();
}
...
function removeLauncher() {
  const old = document.getElementById(LAUNCHER_ID);
  if (old) old.remove();
}

  function ensureLauncher(onClick) {
    // STRICT: launcher should exist ONLY on /live-radio
    if (!isLiveRadioUrl()) { removeLauncher(); return; }
...
// Create/remove launcher based on current route (STRICT)
ensureLauncher(onLauncherClick);
...
`;
    ...
    const btnJsonDl = document.createElement("button");
    btnJsonDl.className = "ncz-miniBtn";
    btnJsonDl.textContent = "Downloader";
    btnJsonDl.title = "Open JSON Media Downloader";
    ...
    rightBtns.append(btnJsonDl, btnRescan, btnClose);
...
function onRouteMaybeChanged(force = false) {
  ...
  if (!routeActive && now) {
    routeActive = true;
    hidden = false;
    ensureLauncher(onLauncherClick);   // ✅ re-create when entering live-radio
    applyVisibility();
    startMirroring();
    ...
    applyVisibility();
    stopMirroring();
    removeLauncher();                  // ✅ remove when leaving live-radio
  } else {
    routeActive = now;
    if (routeActive) ensureLauncher(onLauncherClick);
    else removeLauncher();
    applyVisibility();
  }
}
// ✅ NEW: chat mounts without URL change; keep re-checking until it exists
setInterval(() => {
  const now = isLiveRadioContext();
  if (now !== routeActive) onRouteMaybeChanged(true);
}, 750);
...
(() => {
  const BRIDGE_FLAG = "__ncz_json_downloader_bridge_loaded__";
  ...
  window.addEventListener("message", (event) => {
    ...
    if (msg.type === "NCZ_DOWNLOAD_URL_REQUEST") {
      chrome.runtime.sendMessage(
        {
          type: "NCZ_DOWNLOAD_URL",
          url: msg.url,
          filename: msg.filename
        },
        (response) => {
          const reply = {
            type: "NCZ_DOWNLOAD_URL_RESPONSE",
          ...
          if (chrome.runtime.lastError) {
            ...
            reply.ok = true;
            reply.downloadId = response.downloadId;
            reply.error = null;
          } else {
            reply.error = response?.error || "Download failed.";
          }
